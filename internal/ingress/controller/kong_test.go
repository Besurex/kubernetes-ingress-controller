	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				...
				customEntitiesJSONBytes: []byte(`{"my-custom-dao-name":[{"name":"custom1","key1":"value1"},{"name":"custom2","dumb":"test-value","boring-test-value-name":"really?"}]}`),
			},
			want:    []byte(`{"_format_version":"1.1","my-custom-dao-name":[{"key1":"value1","name":"custom1"},{"boring-test-value-name":"really?","dumb":"test-value","name":"custom2"}],"services":[{"host":"example.com","name":"foo"}]}`),
			wantErr: false,
		},
	}
